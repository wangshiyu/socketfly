package wsy.clientCore.queue.receiveQueue;
...
public class ReceiveQueue extends Thread{//这里注意初始化的顺序
	private static Logger log=Logger.getLogger(ReceiveQueue.class); 
	private CommandExecute commandExecute=PropertiesCofig.getCommandClientExecute();
	private Queue queue;
	public ReceiveQueue() {
		super();
		queue = new Queue(PropertiesCofig.getReceiveQueueSize());
		log.info("客户端信息接收队列初始化完成！");
	}
			...
			if (!isEmpty()&&PropertiesCofig.getCommandExecuteIsOpen()) {
				...
				try {				
					if(null!=receiveJMessage.getCommandExecute()){
						...
						poll();// 去除头元素
					}else if(commandExecute!=null){
						commandExecute.execute(receiveJMessage);// 命令执行接口
						poll();// 去除头元素
					}else{
